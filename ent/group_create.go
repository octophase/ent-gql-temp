...
func (gc *GroupCreate) defaults() error {
	if _, ok := gc.mutation.Username(); !ok {
		v := group.DefaultUsername
		gc.mutation.SetUsername(v)
	}
	return nil
}
...
func (gc *GroupCreate) check() error {
	...
	if _, ok := gc.mutation.Username(); !ok {
		return &ValidationError{Name: "username", err: errors.New(`ent: missing required field "Group.username"`)}
	}
	if _, ok := gc.mutation.TenantID(); !ok {
...
func (gc *GroupCreate) createSpec() (*Group, *sqlgraph.CreateSpec) {
	...
	if value, ok := gc.mutation.Username(); ok {
		_spec.SetField(group.FieldUsername, field.TypeString, value)
